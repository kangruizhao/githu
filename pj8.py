...
def process_sprite_group(sprites, canvas):
    ...
    #remove Sprites that have spired
    sprites.difference_update(sprites_to_remove)
def group_collide(sprites,ship):
    collisions = 0
    sprites_to_remove=set([])
    #print type(ship)
    for sprite in sprites:
        if sprite.collide(ship):
            collisions+=1
            sprites_to_remove.add(sprite)
            new_explosion = Sprite(ship.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
...
def group_group_collide(rocks, missiles):
    ...
    for sprite in rocks:
        if group_collide(missiles, sprite) > 0:
            collisions += 1
            sprites_to_remove.add(sprite)
            
    # remove Sprites that have collided
    rocks.difference_update(sprites_to_remove)        
...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            image_center_aux = [self.image_center[0] + (self.age * self.image_size[0]), self.image_center[1]]
            canvas.draw_image(self.image, image_center_aux, self.image_size,
                              self.pos, self.image_size, self.angle)
        canvas.draw_image(self.image, self.image_center, self.image_size,
                          self.pos, self.image_size, self.angle)
    ...
    def collide(self, other_object):
        ...
        return distance < self.get_radius() + other_object.get_radius();
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        while dist(rock_pos,my_ship.get_position())<my_ship.get_radius():
            rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
        rock_avel = random.random() * .2 - .1
        new_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
        
        # check that the new rock doesn't collide with the Ship
